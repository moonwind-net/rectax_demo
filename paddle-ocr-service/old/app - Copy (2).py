...
# 预编译正则表达式（避免每次调用都编译）
_RE_COMPILATIONS = {
    "REG": re.compile(r"T\d{13}"),
    "DATE": re.compile(r"(\d{4})[-/:年]\s*(\d{1,2})[-/:月]\s*(\d{1,2})[-/:日]"),
    "PHONE": re.compile(r"(\d{1,4})[-/]\s*(\d{1,4})[-/]\s*(\d{4})"),
    "NUMBERS": re.compile(r"[\d,.]+"),
    "TAX_MARK": re.compile(r"[税|税込|税抜]"),
}

# 性能配置常量
_PERF_CONFIG = {
    "MAX_AMOUNT_CANDIDATES": 15,           # 最多只处理15个金额候选
    "MIN_TOTAL_AMOUNT": 100,               # 总金额候选最小值
    "MIN_TENDER_AMOUNT": 1000,             # 支付金额候选最小值
    "MAX_CHANGE_AMOUNT": 10000,            # 找零金额候选最大值
    "TAX_RATES": [10, 10.5],               # 日本税率
    "TOLERANCE_RATIO": 0.01,               # 金额容差比例（1%）
}
...
def _build_amount_role_candidates(entries: List[Dict[str, Any]], 
                                  tax_rates: List[float] = None) -> List[Dict[str, Any]]:
    """
    优化版：金额角色候选生成
    原复杂度 O(N²) → 现复杂度 O(N)
    优化点：
    1. 预筛选候选（只保留可能的总额/支付金额）
    2. 限制候选数量
    3. 使用整数避免浮点误差
    4. 提前break失败匹配
    """
    ...
    tax_rates = tax_rates or _PERF_CONFIG["TAX_RATES"]
    candidates = []
    
    # ========== 第一步：金额预处理（转整数，避免浮点误差） ==========
    processed_entries = []
    ...
    # ========== 第二步：预筛选候选（只保留可能的角色） ==========
    # 按金额降序排序，总额通常在底部且金额较大
    processed_entries.sort(key=lambda x: x["int_amount"], reverse=True)
    
    # 筛选总额候选：金额较大，且置信度高
    total_candidates = [
        e for e in processed_entries 
        if e["int_amount"] >= _PERF_CONFIG["MIN_TOTAL_AMOUNT"]
    ][: _PERF_CONFIG["MAX_AMOUNT_CANDIDATES"]]
    
    # 筛选支付候选：整百/整千（常见日元面额）
    tender_candidates = [
        e for e in processed_entries
        if e["int_amount"] >= _PERF_CONFIG["MIN_TENDER_AMOUNT"] 
        and (e["int_amount"] % 1000 == 0 or e["int_amount"] % 100 == 0)
    ][: _PERF_CONFIG["MAX_AMOUNT_CANDIDATES"]]
    
    # ========== 第三步：配对匹配（O(N) 复杂度） ==========
    for total_candidate in total_candidates:
        int_total = total_candidate["int_amount"]
        
        # 尝试匹配支付金额
        for tender_candidate in tender_candidates:
            int_tender = tender_candidate["int_amount"]
            
            # 支付必须 >= 总额
            if int_tender < int_total:
                continue
            
            # 计算找零
            int_change = int_tender - int_total
            
            # 验证找零是否在合理范围内
            if int_change < 0 or int_change > _PERF_CONFIG["MAX_CHANGE_AMOUNT"]:
                continue
            
            # 计算税率容差（允许1%的误差）
            tolerance = int(int_total * _PERF_CONFIG["TOLERANCE_RATIO"])
            
            # 检查是否存在对应的找零金额
            change_match = None
            for entry in processed_entries:
                if entry["original_idx"] == total_candidate["original_idx"] or \
                   entry["original_idx"] == tender_candidate["original_idx"]:
                    continue
                    
                # 允许±tolerance 的误差
                if abs(entry["int_amount"] - int_change) <= tolerance:
                    change_match = entry
                    break
            ...
            for rate in tax_rates:
                tax_amount = int_total - int_total // (100 + rate) * 100
                if tax_amount <= 0:
                    continue
                    
                # 检查是否存在对应税额
                for entry in processed_entries:
                    if entry["original_idx"] in [total_candidate["original_idx"], 
                                                  tender_candidate["original_idx"]]:
                        continue
                    if change_match and entry["original_idx"] == change_match["original_idx"]:
                        continue
                    
                    if abs(entry["int_amount"] - tax_amount) <= tolerance:
                        tax_valid = True
                        break
...
def _extract_registration_number_from_roi(image_np: np.ndarray,
                                          roi_coords: Dict[str, int]) -> Optional[str]:
    """
    优化版：从ROI区域提取注册号
    优化点：
    1. 使用lru_cache缓存预处理后的图像
    2. 减少阈值迭代次数
    3. 提前返回不满足尺寸的区域
    """
    import cv2
    ...
    # 验证ROI坐标是否有效
    if not all(k in roi_coords for k in ["x", "y", "w", "h"]):
        return None
    
    x, y, w, h = roi_coords["x"], roi_coords["y"], roi_coords["w"], roi_coords["h"]
    
    # 提前返回：区域太小不可能包含注册号（T + 13位数字）
    if w < 100 or h < 20:
        return None
    ...
    # ========== 优化：Lru Cache 缓存预处理后的图像 ==========
    _cached_ocr = None
    
    def _cached_ocr_engine(img, config=None):
        nonlocal _cached_ocr
        if _cached_ocr is None:
            # 首次调用初始化
            _cached_ocr = ocr_engine.ocr(img, cls=True, det=True)[0]
        return _cached_ocr
    
    # ========== 优化：简化阈值迭代（只尝试2种阈值） ==========
    thresholds = [127, 180]  # 原代码尝试5-10种，这里只保留最常用的2种
    ...
    for threshold in thresholds:
        ...
        # 尝试OCR识别
        try:
            ocr_result = _cached_ocr_engine(inverted)
        except:
            continue
        ...
        if ocr_result:
            for box in ocr_result:
                ...
                text = box[1][0] if isinstance(box[1], tuple) else box[1]
                if not text:
                    continue
                
                # 匹配注册号格式
                match = _RE_COMPILATIONS["REG"].search(text)
                if match:
                    return match.group(0)
