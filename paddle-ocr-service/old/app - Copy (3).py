...
# Performance tuning for registration ROI processing
# Controls upscaling factor applied to ROI before thresholding (smaller -> faster)
REGISTRATION_ROI_UPSCALE = int(os.getenv("REG_ROI_UPSCALE", "2"))
# Comma-separated threshold list used for simple binarization passes
try:
    REGISTRATION_ROI_THRESHOLDS = [int(x) for x in os.getenv("REG_ROI_THRESHOLDS", "150,190").split(",") if x.strip()]
except Exception:
    REGISTRATION_ROI_THRESHOLDS = [150, 190]
# Inverted thresholds to try on inverted image
try:
    REGISTRATION_ROI_INVERT_THRESHOLDS = [int(x) for x in os.getenv("REG_ROI_INVERT_THRESHOLDS", "150").split(",") if x.strip()]
except Exception:
    REGISTRATION_ROI_INVERT_THRESHOLDS = [150]
# Maximum number of candidate images to send to OCR per ROI (including original)
REGISTRATION_MAX_CANDIDATES = int(os.getenv("REG_ROI_MAX_CANDIDATES", "5"))
# Control scan order: 'lines_first' (faster, tries OCR lines before ROI) or 'roi_first' (more thorough)
REGISTRATION_SCAN_ORDER = os.getenv("REGISTRATION_SCAN_ORDER", "lines_first").lower()

# Precompile registration regexes for speed
REGISTRATION_INLINE_RE = re.compile(r"T\d{13}")
REGISTRATION_CANDIDATE_RE = re.compile(r"([T1]?\d{13,14})")
...
def _extract_total_amount(lines: list[str], line_layout_meta: list[dict[str, float | int]] | None = None) -> float:
    ...
    if len(scored_candidates) > 1:
        ...
        for alt_score, alt_amount, _ in scored_candidates[1:6]:
            ...
            has_leading_one_inflation = (
                len(top_text) == len(alt_text) + 1
                and top_text.startswith("1")
                and top_text[1:] == alt_text
            )
            if not has_leading_one_inflation:
                continue
            ...
            if small_hits >= 2 and (top_score - alt_score) <= 30 and 100 <= alt_amount <= 100000:
                return alt_amount
...
def _extract_registration_number_from_lines(lines: list[str]) -> str | None:
    ...
    for idx, line in enumerate(lines):
        ...
        match = REGISTRATION_INLINE_RE.search(line)
        if match:
        ...
        if any(keyword in normalized for keyword in REGISTRATION_LABEL_KEYWORDS):
            # Try same line first
            match = REGISTRATION_CANDIDATE_RE.search(_normalize_registration_whitelist_text(line))
            if match:
            ...
            if idx + 1 < len(lines):
                ...
                match = REGISTRATION_CANDIDATE_RE.search(next_line)
                if match:
                    result = _parse_registration_candidate(match.group(1))
                    if result:
                        return result
...
def _build_registration_roi_candidates(image_np: np.ndarray) -> list[np.ndarray]:
    ...
    # Use configurable upscale factor to trade quality for speed
    upscaled = image.resize(
        (max(image.width * REGISTRATION_ROI_UPSCALE, 1), max(image.height * REGISTRATION_ROI_UPSCALE, 1)),
        resample=Image.BICUBIC,
    )

    candidates: list[np.ndarray] = []
    # Original (upscaled) image first
    try:
        candidates.append(np.array(upscaled.convert("RGB")))
    except Exception:
        candidates.append(np.array(upscaled))

    # Add a small number of thresholded variants (bounded by REGISTRATION_MAX_CANDIDATES)
    count = len(candidates)
    for threshold in REGISTRATION_ROI_THRESHOLDS:
        if count >= REGISTRATION_MAX_CANDIDATES:
            break
        ...
        try:
            ...
            candidates.append(np.array(thresholded))
        count += 1

    # Try a few inverted thresholds as well, if we still have budget
    inverted = ImageOps.invert(upscaled)
    for threshold in REGISTRATION_ROI_INVERT_THRESHOLDS:
        if count >= REGISTRATION_MAX_CANDIDATES:
            break
...
def _extract_registration_number(
    ...
    # Prefer a cheap line-based extraction first when configured to do so.
    if REGISTRATION_SCAN_ORDER == "lines_first":
        line_candidate = _extract_registration_number_from_lines(lines)
        if line_candidate:
            return line_candidate, "line", []
    ...
    # Default: ROI-first behavior (more thorough but slower)
    roi_candidate, roi_source, roi_candidates = _extract_registration_number_from_roi(line_entries, images_np)
    if roi_candidate:
        return roi_candidate, roi_source, roi_candidates
...
def _to_receipt_payload(
    ...
    # Precompute normalized total keywords for strict-keyword overrides
    normalized_total_keywords = tuple(_normalize_matching_text(k) for k in STRICT_TOTAL_KEYWORDS)
